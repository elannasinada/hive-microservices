...
const Register = () => {
  ...
  const validate = () => {
    ...
    // Education and major validation
    if (!education || education.length < 2) newErrors.education = 'Education must be at least 2 characters';
    if (!major || major.length < 2) newErrors.major = 'Major must be at least 2 characters';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const success = await register(email, password, username, Number(age), education, major);
      if (success) {
        navigate('/login');
      }
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-primary mb-2">Hive</h1>
